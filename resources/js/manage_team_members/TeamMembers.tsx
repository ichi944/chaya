...
interface Props {
  ...
  history: any;
  initialize: () => void;
  handleVerifyByAdmin: (memberId: number) => void;
  handleLockMember: (memberId: number) => void;
  handleUnlockMember: (menberId: number) => void;
  handleToggleShowMembersWhoIsNotVerifiedWithEmail: (isInputChecked: boolean) => void;
}
class TeamMembers extends React.Component<Props> {
  ...
  render() {
    const { members, show_members_who_is_not_verified_with_email } = this.props.teamMembers;
    const { authorizationToken } = this.props.auth;
    return (
      <div>
        <Button style={styles.backButton} onClick={this.handleClickBack}>
          back
        </Button>
        <Card style={styles.headerCard}>
          <CardHeader
            title="メンバーの管理を行う"
            subheader="各メンバーのステータスの確認と認証、ロック/アンロックを行います。"
          />
          <CardActions>
            <FormControlLabel
              control={
                <Switch
                  checked={show_members_who_is_not_verified_with_email}
          ...
          <CardContent>
            メンバーのプロフィールとステータスの確認・変更を行うことが出来ます。
            <br />
            メンバーのログインには、メール認証を行なったのち管理者による承認が必要です。
            <br />
            メンバーをロックするとそのメンバーはログイン出来なくなります。投稿はそのまま残ります。
          </CardContent>
        </Card>
        <Paper className="article_index-container">
          <Subheader title="メンバー一覧" />
          <Divider />
          {members.map(member => {
            return member.is_verified_with_email || show_members_who_is_not_verified_with_email ? (
              <Paper key={member.id!}>
